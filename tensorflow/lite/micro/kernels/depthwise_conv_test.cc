...
void TestDepthwiseConvFloat(const int32_t* input_dims_data, const float* input_data,
  ...
  constexpr int outputs_size = 1;
  constexpr int tensors_size = inputs_size + outputs_size;
  tensors[0] = CreateFloatTensor(input_data, input_dims, "input_tensor");
  tensors[1] = CreateFloatTensor(filter_data, filter_dims, "filter_tensor");
  tensors[2] = CreateFloatTensor(bias_data, bias_dims, "bias_tensor");
  tensors[3] = CreateFloatTensor(output_data, output_dims, "output_tensor");
...
void TestDepthwiseConvQuantizedPerLayer(
  ...
  int32_t filter_zero_points[] = {1, 128};
  ...
  int32_t bias_zero_points[] = {1, 128};
...
void TestDepthwiseConvQuantizedPerChannel(
    ...
    const int32_t* filter_dims_data, const float* filter_data,
    int8_t* filter_data_quantized, const int32_t* bias_dims_data,
  ...
  int32_t filter_zero_points[kMaxFilterChannels];
  float filter_scales[kMaxFilterChannels];
  int32_t bias_zero_points[kMaxBiasChannels];
  ...
  tensors[1] = CreateSymmetricPerChannelQuantizedTensor(
  ...
  int32_t input_zero_points[] = {1, input_zero_point};
  ...
  int32_t output_zero_points[] = {1, output_zero_point};
  TfLiteAffineQuantization output_quant = {
      FloatArrayFromFloats(output_scales),
      IntArrayFromInts(output_zero_points)};
  tensors[3].quantization = {kTfLiteAffineQuantization, &output_quant};
...
TF_LITE_MICRO_TEST(SimpleTestOptimizedFilterWidth) {
  ...
  const int32_t input_shape[] = {4, 1, 1, 9, 1};
  const int32_t filter_shape[] = {4, 2, 1, 8, 1};
  const int32_t bias_shape[] = {1, 1};
  const float goldens[] = {
      92, 56, 12, 22, 33, 72, 44, 20, 5,
  };
  const int32_t output_shape[] = {4, 1, 1, 9, 1};
...
TF_LITE_MICRO_TEST(SimpleTestQuantizedPerChannel) {
  ...
  int32_t zero_points[bias_elements + 1];
...
TF_LITE_MICRO_TEST(SimpleTestQuantizedPerChannelDepthMultiplier1) {
  ...
  const int32_t filter_shape[] = {4, 1, 2, 2, 2};
  const float filter_values[] = {1, 2, 3, 4, -9, 10, -11, 12};
  const int bias_elements = 2;
  const int32_t bias_shape[] = {4, 1, 1, 1, 2};
  ...
  const int32_t output_shape[] = {4, 1, 2, 1, 2};
...
TF_LITE_MICRO_TEST(TestQuantizedPerChannelDepthMultiplier1Relu6) {
  ...
  const int32_t input_shape[] = {4, 1, 3, 2, 4};
...
TF_LITE_MICRO_TEST(TestQuantizedPerChannelCompareWithFloat) {
  const int32_t input_dims[] = {4, 1, 2, 3, 2};
  const float input_data[] = {3, 2, 1, -1, -2, -3, 4, 3, 2, -2, -3, -4};
  const int32_t filter_dims[] = {4, 1, 2, 2, 4};
  const float filter_data[] = {1, 2, 3, 4, 3, 4, 5, 6, 7, 8, 5, 6, 3, 4, 1, 2};
  const int32_t bias_dims[] = {4, 1, 1, 1, 4};
  const float bias_data[] = {3, -2, 4, 6};
  const int32_t output_dims[] = {4, 1, 1, 2, 4};
  ...
  int32_t zero_points[bias_size + 1];
...
TF_LITE_MICRO_TEST(FilterDimsNotMatchingAffineQuantization) {
  ...
  const int32_t bias_shape[] = {4, 1, 1, 1, 4};
  const float bias_data[] = {3, -2, 4, 6};
  const int32_t output_shape[] = {4, 1, 1, 2, 4};
  const float golden[] = {43, 48, 18, 22, 3, -4, -28, -36};
  ...
  constexpr int outputs_size = 1;
  constexpr int tensors_size = inputs_size + outputs_size;
  tflite::testing::tensors[0] = input_tensor;
  tflite::testing::tensors[1] = filter_tensor;
  tflite::testing::tensors[2] = bias_tensor,
  tflite::testing::tensors[3] = output_tensor;
